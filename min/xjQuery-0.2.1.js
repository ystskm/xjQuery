...
(function(window, $) {
  ...
  $xj.add('ComUtil', {
    ...
    uaAnalyze: function(ua, criteria) {
      ...
      } else if(mtc = ua.match(/(\s|^)([\w-]+bot[\w-]*)\/(\d+(\.\w+))(\s|;|$)/)) {
      ...
      if(!bwsver) {
        mtc ? bwsver = mtc[1].slice(bws.length + 1): console
            .warn('Unrecognized browser version: ' + ua)
      }
