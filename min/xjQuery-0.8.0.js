...
(function(has_win, has_mod) {
  ...
  if(exports.xjQuery)
    return;
  exports.xjQuery = xjQuery, exports.$xj = $xj;
  has_mod && (module.exports = xjQuery);

  var f = exports.foonyah; // optional plugged library: foonyah
  var $ = exports.jQuery || exports.$;

  // debug function set if debug.js is Loaded.
  var debug = exports.debug ? exports.debug('xjQuery'): $.noop;

  // internal event name creator
  var internalEmitter = $({}), internalEventName = function(type) {
    return !f ? type: f.internalEventName(type);
  };
  ...
  xjQuery.isFunction = isFunction;

  function isFunction(fn) {
    return $.isFunction(fn);
  }

  xjQuery.isArray = isArray;
  ...
  function xjQuery() {
    ...
    var typ0 = $.type(arg0), typ1 = $.type(arg1);

    // wait prepare mode
    if('function' == typ1 && ('string' == typ0 || isArray(arg0))) {
      ...
      for( var i = 0; i < places.length; i++)
        (function(dfd, place) {
          var idx = !f || place.indexOf(f.Prefix.Plugin);
          var len = f.Prefix.Plugin.length;

          if(idx == 0) {
            // resolve over foonyah
            f(place.substr(len)).then(dfd.resolve);
            return;
          }

          if(!xjQuery.data('readystatus', places[i])) {
            // resolve over xjQuery
            xjQuery.one('ready', places[i], dfd.resolve);
            return;
          }

          dfd.resolve();
        })(dfds[dfds.push($.Deferred()) - 1], places[i]);
      return $.when.apply($, dfds).then(fn);
    ...
    if('function' == typ0 && f) {
      var evt = f.internalEventName('load');
      return f.isStarted() ? arg0(): $(exports).one(evt, arg0);
    }
  ...
  var getters = {
    ...
    version: '',
    release: '',
    workon: $.type(exports.process) == 'undefined' ? 'Browser': 'Node',
  ...
  $.each(getters, function(k, v) {
    (function(rule) {
      Object.defineProperty(xjQuery, k, rule);
      Object.defineProperty($xj, k, rule);
    })({
      get: callparameter(v),
      set: prohibitsetter(k)
    });
  });
...
(function(has_win) {
  ...
  (function() {
    ...
    function initialize() {
      console.error('$xj.Chain is deprecated. use foonyah.chain.');
  ...
  (function() {
    ...
    function when(val) {
      console.error('$xj.when is deprecated. use Promise.all.')
  ...
  (function() {
    ...
    function initialize(files, options, callback) {
      console.error('$xj.require is deprecated.')
  ...
  (function() {
    ...
    var ua = foonyah.userAgent(exports.navigator['userAgent']);
...
}).call(this, typeof window != 'undefined');