...
for(var e in a)e!=b&&(d.prototype[e]=a[e]);return d}});l.CLASS.makeClass.fn=l.CLASS.makeClass.prototype={init:function(a){this[l.CLASS.Constants.definition]=a}};var k={initialize:function(){this._emitter=new foonyah.Emitter;this._mystart=!1;this._dfd=g.Deferred();this._position=0;this._actors=[];this._witharg=[];this._unstop=!1;this.push.apply(this,m(arguments))},nextTick:"undefined"!=typeof process&&process.nextTick?process.nextTick:function(a){setTimeout(a,0)},unstopping:function(a){this._unstop=
!0===a;return this},withArg:function(){this._witharg=this._witharg.concat(m(arguments));return this},push:function(){var a=m(arguments);if(this.unstop)for(var c=0;c<a.length;c++)this._next_unstop(a[c]);else for(c=0;c<a.length;c++)this._next(a[c]);return this},start:function(){this._mystart=!0;if(0==this._actors.length)return this._dfd.resolve().promise();this._actors[this._position].apply(this,m(arguments));this.trigger("start");return this._dfd.promise()},length:function(){return this._actors.length},
isStarted:function(){return!0===this._mystart},isEnded:function(){return!0===this._mystart&&(0==this._actors.length||this._position==this._actors.length-1)},_next:function(a){var c=this;this._actors.push(function(){var b=m(arguments);0==b.length&&(b=[null]);var d=b.shift();a.apply(c,[d].concat(c._witharg).concat(b).concat(c._callback()))})},_next_unstop:function(a){var c=this;this._actors.push(function(){var b=m(arguments);0==b.length&&(b=[null]);var d=b.shift();a.apply(c,[d].concat(c._witharg).concat(b).concat(c._callback_unstop()))})},
_callback:function(){var a=this;return function(c){var b=m(arguments);if(c)return a._error.apply(a,b);a.nextTick(a._execNextFn(b))}},_callback_unstop:function(){var a=this;return function(c){var b=m(arguments);a.nextTick(a._execNextFn(b))}},_execNextFn:function(a){var c=this;0==a.length&&(a=[null]);return function(){try{if(c._actors[++c._position])return c._actors[c._position].apply(c,a);if(c._actors.length==c._position)return c.trigger("end"),c._dfd.resolve();("undefined"==typeof foonyah?console.log:
foonyah.log)("Duplicate next() call may occurs somewhere in $xj.Chain.")}catch(b){return a=m(a),a.shift(),a.unshift(b),c._error.apply(c,a)}}},_error:function(a){var c=m(arguments),b=null;this._dfd.reject(a);var d=this._actors.length-1;this._position<d&&(b=this._actors[d]);if("function"==typeof b)return b.apply(this,c);if(a instanceof Error)throw a;throw Error(a);}};(function(a,c){"bind,unbind,on,one,off,trigger".split(",").forEach(function(b){c[b]=function(c){this._emitter[b](l.eventName(c,a))}})})("Chain",
k);o.add("Chain",l.CLASS.makeClass(k));o.add("when",function(a){function c(a){return a?a&&a.state?a:(new g.Deferred).resolve(a):(new g.Deferred).reject(a)}return g.when.apply(g,g.isArray(a)?a.map(c):[c(a)])});o.add("require",l.CLASS.makeClass({constant:function(a){return{ManageKey:"requre-",StorageKey:"require"}[a]},initialize:function(a,c,b){function d(a){var b=e.constant("ManageKey").length;return a.slice(0,b)==e.constant("ManageKey")}"function"==typeof c&&(b=c,c={});if(!(this instanceof l.require))return(new l.require(c)).next(a).start(b);
g.isPlainObject(a)||(a={});var e=this;this.options=g.extend({removeOnLoad:!0,evalMode:!1,packageName:this.constant("StorageKey"),versionKey:null},c);this.chain=new o.Chain;this.finish=function(a,b){g.isFunction(e.options.finish)?e.options.finish(a,b):b()};if(!d(this.options.packageName))this.options.packageName=[this.constant("ManageKey"),this.options.packageName].join("");if(this.options.versionKey){this.codeUpdate=!1;try{this.options.srcManager=JSON.parse(localStorage.getItem(this.options.packageName).replace(/\\"/g,
'"'))}catch(f){}try{if(!this.options.srcManager){for(var j in localStorage)d(j)&&localStorage.removeItem(j);this.options.srcManager={}}this.options.versionString=[this.options.packageName,"-",c.versionKey].join("")}catch(h){}}else try{for(j=0;j<localStorage.length;j++){var p=localStorage.key(j);RegExp(this.options.packageName).test(p)&&localStorage.removeItem(p)}this.options.srcManager={}}catch(k){}return this},start:function(a){return this.chain.push(a||this.finish).start()},next:function(){for(var a=
[],c=g.isArray(arguments[0])?arguments[0]:m(arguments),b=0;b<c.length;b++)"string"==typeof c[b]&&(a=a.concat(c[b].split(/\s/)));if(a.length){for(var c=[],d=[],b=0;b<a.length;b++)d.push(new g.Deferred),c.push(this.assign(a[b],d[b]));this.chain.push(this._listenFn(c,g.when.apply(g,d)))}return this},_listenFn:function(a,c){var b=this;return function(d,e){c.done(function(){try{localStorage.setItem(b.options.packageName,JSON.stringify(b.options.srcManager))}catch(a){}e(d)}).fail(function(a){f("Phase on require:_fail()",
"error");for(var b in a){var c=a[b];c instanceof Error?(f(c.message,"error"),c.stack&&f(c.stack,"error")):g.isPlaneObject(c)?f(JSON.stringify(c)):f(c,"error")}});for(var i=0;i<a.length;i++)setTimeout(a[i],0)}},assign:function(a,c){var b=this;switch(typeof a){case "string":return function(){b.load(a,function(b){f(a+" is Loaded."+(!0===b?"(timeout message)":""));c.resolve()},function(){f("fail to load script : "+a,"error");c.resolve()})};case "function":return function(b){try{a.call(),b.resolve()}catch(c){b.reject()}}}},
load:function(a,c,b){function d(b){var c=location.protocol,d=location.host,h=location.port||(/^http:/.test(c)?80:443),l=null,m=(l=a.match(/(^https?:)/))?l[1]:c,n=(l=a.match(/\/\/([^\/]+)\//))?l[1]:d,o=(l=n.match(/:(\d+)$/))?l[1]:/^http:/.test(m)?80:443;k.evalMode&&c==m&&d==n&&h==o?b?i(b):g.ajax({type:"get",url:a,dataType:"text",success:e}):(k.evalMode&&f("xjQuery Notice: "+a+" is read by tag."),j())}function e(a){i(a)}function i(b){var e=!1;l[a]=n;localStorage.setItem([n,".",a].join(""),b);try{g.globalEval(b)}catch(d){e=
!0,f("Error from JSLoader : misssing eval. src = "+a)}e?j():(c||g.noop)()}function j(){var e=k.append||document.body||document.documentElement,d=document.createElement("script");d.src=a;d.type=k.type?k.type:"text/javascript";d.onload=function(){!0===m&&e.removeChild(d);c.apply(this,arguments)};d.onerror=function(){!0===m&&e.removeChild(d);b.apply(this,arguments)};if(k.charset)d.charset=k.charset;if(document.all)d.onreadystatechange=function(){switch(d.readyState){case "complete":case "loaded":d.onload();
break;case "error":d.onerror()}};e.appendChild(d)}var h;h=null;var k=this.options,l=k.srcManager,m=k.removeOnLoad,o=k.packageName,n=k.versionString,o=null;if(o=a.match(/\.-([\w\.-]+)-pkg/))o=o[1],a=a.replace(/\.-([\w\.-]+)-pkg/,""),n=n.replace(/^\w+-/,[o,"-"].join(""));(o=a.match(/\.-([\w\.-]+)-ver/))?(n=o[1],a=a.replace(/\.-([\w\.-]+)-ver/,""),n=n.replace(/-\w+$/,["-",n].join(""))):/\.no-version/.test(a)&&(a=a.replace(/\.no-version/,""),h=!0);if(!h&&n)if(h=[n,".",a].join(""),h=localStorage.getItem(h))f(a+
" is found on localStorage!"),d(h);else{if(h=l[a])localStorage.removeItem([h,".",a].join("")),f(a+" is deleted from localStorage.");d()}else d()}}));o.add("LAYOUT",{layCenter:function(a,c){function b(a,b){var c;try{c=a.css(b)}catch(d){}return!c||isNaN(aft=c.replace("px"))?a[b]():parseInt(aft)}var d=(b(a,"width")-b(c,"width"))/2,e=(b(a,"height")-b(c,"height"))/2;return{left:0<d?d:0,top:0<e?e:0}}});n.$xj.add("JSCSS",{crossCssAuto:function(a){"boolean"==typeof a&&(j=a);return j},crossCss:t,codeAdjust:s,
codeToRgb:function(a){a=s(a).replace(/^#/,"");return[parseInt(a.slice(0,2),"16"),parseInt(a.slice(2,4),"16"),parseInt(a.slice(4,6),"16")]}});var j=!0,k=o.uaAnalyze(n.navigator.userAgent);g.extend(l.JSCSS,k);g.extend(o,k);var v=g.fn.css;g.fn.css=function(){if(!1===j||"string"==typeof arguments[0]&&1==arguments.length)return v.apply(this,arguments);var a={};"string"==typeof arguments[0]?a[arguments[0]]=arguments[1]:a=arguments[0];return v.call(this,t(a))};if("undefined"==typeof WebSocket&&"undefined"!=
typeof MozWebSocket)n.WebSocket=MozWebSocket})(window,window.$);