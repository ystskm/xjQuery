  ...
  && (function(has_win) {
    ...
    (function($, xjQuery, $xj) {
      ...
      function uaAnalyze(ua, criteria) {
        ...
        if(mtc = ua.match(/(Chrome|CriOS)[\s\/](\d+\.\d)/))
          bws = "Chrome", bwsver = mtc[2];
        else if(mtc = ua.match(/Opera[\s\/](\d+\.\d{1,2})/))
          bws = "Opera", bwsver = mtc[1];
        else if(mtc = ua.match(/MSIE[\s\/](\d+\.\d{1,2})/))
          bws = "MSIE", bwsver = mtc[1];
        else if(mtc = ua.match(/Trident\/.+rv:(\d+\.\d{1,2})/))
          bws = "MSIE", bwsver = mtc[1];
        else if(mtc = ua.match(/Firefox[\s\/](\d+\.\d)/))
          bws = "Firefox", bwsver = mtc[1];
