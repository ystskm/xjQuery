...
window.xjQuery
  ...
  || (function(window, foonyah) {
    ...
    var internalEmitter = $({});
    var internalEventName = typeof foonyah == 'undefined' ? function(type) {
      return type;
    }: foonyah.internalEventName;
  ...
  })(window, window.foonyah);

(function(window, foonyah) {

  // =========== closure variables ============ //
  var show_message = typeof foonyah != 'undefined'
    && foonyah.ShowMessages === true ? true: false, _console = function(msg, fn) {
    if(fn == null)
      fn = 'log';
    if(typeof fn == 'string') {
  ...
  $xj.add('ComUtil', {
    ...
    uaAnalyze: function(ua, criteria) {
      ...
      if(/(Opera[\s\/]\d+\.\d{1,2})/.test(ua))
        bws = "Opera";
      else if(/(MSIE[\s\/]\d+\.\d{1,2})/.test(ua))
        bws = "MSIE";
        ...
        if(/(Version[\s\/]\d+\.\d)/.test(ua))
          bwsver = RegExp.$1.slice("Version".length + 1);
        else if(/AppleWebKit[\s\/]4\d{2}/.test(ua))
          bwsver = 2;
        else if(/\d?(\d{3})\./.test(ua) && parseInt(RegExp.$1) > 533)
          bwsver = 5;
      } else if(/(Sleipnir[\s\/]\d\.\d)/.test(ua))
        bws = "Sleipnir";
      else if(/(Konqueror[\s\/]\d\.\d)/.test(ua))
        bws = "Konqueror";
      else if(/Netscape/.test()) {
        bws = "Netscape";
        if(/(Netscape\d)/.test(ua))
          bwsver = RegExp.$1.slice(bws.length);
        else if(/(Netscape[\s\/]\d\.\d{1,2})/.test(ua))
          ;
      } else if(/(DoCoMo[\s\/]\d+\.\d)/.test(ua)) {
        bws = "DoCoMo";
        device = "cellular";
      } else if(/KDDI/.test(ua)) {
        bws = "KDDI";
        device = "cellular";
      } else if(/(J-PHONE[\s\/]\d+\.\d)/.test(ua)) {
        bws = "J-PHONE";
        device = "cellular";
      } else if(/(SoftBank[\s\/]\d+\.\d)/.test(ua)) {
        bws = "SoftBank";
        device = "cellular";
      ...
      if(!bwsver)
        bwsver = RegExp.$1.slice(bws.length + 1);
      ...
      if(/Linux/.test(ua)) {
        ...
        os = "SunOS";
        if(/(SunOS[\s\/]\d\.\d)/.test(ua))
          osver = RegExp.$1.slice(os.length + 1, os.length + 4);
        os = "Solaris";
        ...
        if(/iPod/.test(ua)) {
          os = "iPod";
          device = "mobile";
        } else if(/iPhone[^\d]+((\d+_?){1,3})/.test(ua)) {
          os = "iPhone";
          osver = RegExp.$1.replace(/_/g, '.');
          device = "mobile";
        } else if(/iPad[^\d]+((\d+_?){1,3})/.test(ua)) {
          os = "iPad";
          osver = RegExp.$1.replace(/_/g, '.');
          device = "tablet";
        } else if(/Mac OS[^\d]+((\d+_?){1,3})/.test(ua)) {
          os = "MacOS";
          osver = RegExp.$1.replace(/_/g, '.');
        } else {
          os = "MacOS";
  ...
  var ChainProto = {
    ...
    nextTick: typeof process != 'undefined' && process.nextTick
      ...
      return function() {
        try {
          ...
          if(self._actors.length == self._position)
            return self.trigger(ChainConst.Event.End);
  ...
  $xj.add('require', xjQuery.CLASS.makeClass({
    ...
    initialize: function(files, options, callback) {
      ...
      if(!(this instanceof xjQuery.require))
        return new xjQuery.require(options).next(files).start(callback);
      ...
      this.finish = $.isFunction(this.options.finish) ? this.options.finish
        : function() {
        };
    ...
    start: function(fn) {
      this.chain.push(fn || this.finish).start();
    },
    next: function() {
  ...
  function crossCss(css, bws) {
    ...
    $.each(['box-shadow', 'border-radius', 'border-top-left-radius',
      ...
      if(val = css[name]) {
        var ext = {};
        ext['-webkit-' + name] = val, ext['-moz-' + name] = val;
        $.extend(css, ext);
      }
